...
It's not a replacement for real design taste; you still have to know how to prompt an AI. It's not a real security review for anything high-stakes, and it's not a lawyer for anything with genuine legal/compliance exposure. It's the baseline that should already be true before any of those bigger conversations happen, so that time spent on real judgment calls isn't spent instead on catching a missing favicon or an unhashed password.
